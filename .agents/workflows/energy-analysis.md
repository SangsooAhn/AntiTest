...
4. **지식 연결 및 병합(Merge)**
   - 지식 지도(Index 노트) 파일([knowledge/README.md](file:///C:/Users/dl/.gemini/antigravity/worktrees/260614_github_news/inspect-repository-files/knowledge/README.md))을 조회하여 기존 명제들의 유사도를 대조함.
   - **명제 병합**: 신규 명제 중 기존 것과 유사한 명제가 이미 존재할 경우, 명제 노트(`knowledge/propositions/주체.md`) 업데이트는 생략(Skip)함. 기존에 없던 명제일 경우에만 해당 주체 명제 노트 하단에 대괄호 제목 형태(`#### [제목]`)로 추가함.
...
5. **Index 노트(지식 지도) 자동 업데이트**
   ...
   - 이 스크립트는 `knowledge/propositions/*.md` 파일들의 명제 제목 리스트와 `knowledge/facts/*.md` 파일들의 사실 개수를 추출하여 지식 지도([knowledge/README.md](file:///C:/Users/dl/.gemini/antigravity/worktrees/260614_github_news/inspect-repository-files/knowledge/README.md))를 자동으로 갱신함.
