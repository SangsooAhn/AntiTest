...
4. **지식 연결 및 병합(Merge)**
   - **인덱스 조회**: 추가하려는 명제의 해당 주제 개별 인덱스 노트(`knowledge/indexes/주제명.md`)를 조회하여 기존 명제들의 제목을 확인함.
   - **명제 병합**: 신규 명제 중 기존 것과 유사한 주장형 제목의 명제가 이미 해당 인덱스 노트에 리스팅되어 있을 경우, 명제 업데이트는 생략(Skip)함. 기존에 없던 명제일 경우에만 `knowledge/propositions/` 아래에 주장형 제목을 가진 마크다운 파일을 개별 신규 생성하여 명제 주장과 논리적 근거를 작성함.
...
5. **Index 노트 및 지식 지도 자동 업데이트**
   ...
   - 이 스크립트는 `knowledge/propositions/*.md` 파일들의 메타데이터와 최종 갱신일들을 파싱하여 개별 주제 인덱스 노트(`knowledge/indexes/*.md`)와 최근 업데이트 로그가 연동된 마스터 지식 지도([knowledge/README.md](file:///C:/Users/dl/.gemini/antigravity/worktrees/260614_github_news/inspect-repository-files/knowledge/README.md))를 자동으로 갱신함.
